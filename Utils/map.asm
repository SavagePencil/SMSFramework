...
Map_Flat_GetCursor:
    ...
    ld  b, (ix + sMapDefinition.MapWidthBitShifts)
    ld  a, b
...
_Map_Flat_GetCursorYPosFound:
    ...
    ld  b, (ix + sMapDefinition.BytesPerEntry)
...
-:
    ...
    ld  c, (ix + sMapDefinition.MapData + 0)
    ld  b, (ix + sMapDefinition.MapData + 1)
    add hl, bc
...
Map_Flat_LoadDataToBlock:
    ...
    push    de
        ; Get X/Y positions
        ld  e, (iy + sMapLoadRequest.MapXPos + 0)
        ld  d, (iy + sMapLoadRequest.MapXPos + 1)
        ld  l, (iy + sMapLoadRequest.MapYPos + 0)
        ld  h, (iy + sMapLoadRequest.MapYPos + 1)
    ...
    ld  b, (iy + sMapLoadRequest.Height + 0)
-:
    push    bc
        push    hl  ; Preserve src
            push    de  ; Preserve dest
                ld  c, (iy + sMapLoadRequest.Width + 0)
                ld  b, (iy + sMapLoadRequest.Width + 1)
                ldir    ; Copy it

            pop hl  ; Confusing!  Get the dest into HL for now.
            ld  c, (iy + sMapLoadRequest.Stride + 0)
            ld  b, (iy + sMapLoadRequest.Stride + 1)
            add hl, bc
            ex  de, hl  ; Now DE points back to our dest.
        pop     hl  ; Restore src loc
        ld      c, (ix + sMapDefinition.MapWidthInBytes + 0)
        ld      b, (ix + sMapDefinition.MapWidthInBytes + 1)
